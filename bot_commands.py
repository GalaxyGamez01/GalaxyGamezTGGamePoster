...
def cmd_help(chat_id, user_id, users):
    if is_admin(user_id):
        text = (
            "ADMIN COMMANDS\n"
            "▶️ Post Now - post now\n"
            "🔄 Refresh - check feeds for new posts\n"
            "⏭️ Skip - skip next unposted post\n"
            "🗑️ Reset History - clear posted history (Advanced)\n"
            "💚 Health - system status\n"
            "📊 Stats - today's stats\n"
            "📜 Logs - latest run logs (Advanced)\n"
            "🧪 Test - send a test message\n"
            "📡 Channels - check channel access\n"
            "⏸️ Pause / ▶️ Resume - pause/resume ALL users\n"
            "📢 Broadcast - send an ad/promo post to every connected channel\n"
        ...
        text = (
            "COMMANDS\n"
            "▶️ Post Now - post now (your channel)\n"
            "🔄 Refresh - check your feed for new posts\n"
            "⏭️ Skip - skip your next unposted post\n"
            "🧪 Test - send a test message\n"
            "📊 Stats - today's stats\n"
            "📡 Channels - check your channel access\n"
            "➕ Add Channel - connect your channel to this bot\n"
            "📰 Add Blog - set the Blogger feed to auto-post from\n"
            "⏸️/▶️ My Channel Pause/Resume - pause/resume your channel only\n\n"
            "⚠️ This bot is FREE to use. In exchange, Galaxy Gamez may "
            "occasionally send sponsored/promo posts to your channel. "
            "Deleting a sponsored post before 4 hours = your channel gets "
            f"removed. 3 strikes = permanent ban.\n\n"
            "Note: commands are checked every ~5 minutes, not instantly."
            + CREDITS_LINE
        )
...
def handle_onboarding_message(chat_id, user_id, users, message):
    ...
    if step == "awaiting_channel":
        ...
        u["channels"].append({
            ...
            "blog_feed_url": "",
            "paused": True,  # stays paused until a blog feed is added
            "posted": [],
        })
    ...
    if step == "awaiting_blog":
        ...
        u["channels"][-1]["paused"] = False
        u["onboarding"]["step"] = None
        send_message(chat_id, "Blog feed set ✅. Your channel will now auto-post every 3 hours.")
        return True
...
def handle_message(message, users):
    ...
    if text in ("/start", "/help") and text == "/start":
        send_message(chat_id, "Welcome to Galaxy Gamez! 🎮" + CREDITS_LINE, reply_markup=keyboard_for(user_id))
